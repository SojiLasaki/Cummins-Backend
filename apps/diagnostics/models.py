...
class StaffReport(models.Model):
    created_by = models.ForeignKey('users.Profile', on_delete=models.SET_NULL, null=True, blank=True, related_name="technician_reports_created")
    LEVEL = (
        ('junior', 'Junior'),
        ('mid', 'Mid'),
        ('senior', 'Senior'),
    ...
    specialization = models.CharField(max_length=50, choices=POSITION_CHOICES, default="engine")
    expertise_requirement = models.CharField(max_length=50, choices=LEVEL, default="junior")
    customer = models.ForeignKey(CustomerProfile, on_delete=models.SET_NULL, null=True, blank=True, related_name="staff_reports")
